...
router.get('/heroi/:id', (req, res, next) => {
    ...
    if(!isNaN(parseInt(id))) {
        ...
        res.json({"error": "Cannot SELECT without id!"});
    }
});
...
router.post('/', (req, res, next) => {
    ...
    if (isNull(idHeroi) || isNull(idPoder)) {
        res.json({error: "Fields 'idHeroi' and 'idPoder' cannot be set to null!"});
    } else {
...
router.put('/', (req, res, next) => {
    ...
    if (isNull(listaPoder.id)) {
        ...
        res.status(400).json({error: "Invalid value for fields 'id_heroi' or 'id_poder'"});
    } else {
